...
class LocationTest extends AnyFunSuite {
  ...
  test("Read Location - missing file") {
    val exception = intercept[NoSuchFileException](readLocations("ttest"))
  }

  test("Read Location - duplicate names") {
    val locationJSON =
      """[
        { "name": "Pub", "coordinates": [-0.1276, 51.5072] },
